...
const createThumbnail = function(thumbnail, index) {
  if (!thumbnail) {
...
class PlaylistMenuItem extends Component {
  ...
  constructor(player, playlistItem, settings, index) {
  ...
  createEl() {
    ...
    // Thumbnail image
    this.thumbnail = createThumbnail(item.thumbnail, this.index);
...
class PlaylistMenu extends Component {
  ...
  constructor(player, settings) {
    ...
    this.on('scroll', throttle((event) => {

      var firstUnloaded = document.querySelector('.vjs-playlist-img-unloaded');
      var offset;

      if (firstUnloaded === null) {
        videojs.log('Fully loaded');
        return;
      } else {
        offset = firstUnloaded.parentNode.parentNode.offsetTop;
        videojs.log(offset);
      }

      // If they have scrolled halfway through the playlist...
      if (this.el_.scrollTop > offset - this.el_.offsetHeight) {
        var loading = false;
        var numToLoad = 0;

        this.$$('.vjs-playlist-item img').forEach((el) => {

          if (!loading && !el.className.match('vjs-playlist-img-unloaded')) {
            numToLoad++;
          } else if (!loading) {
            loading = true;
            videojs.log('loading ' + numToLoad);
          }

          if (loading && numToLoad > 0) {
